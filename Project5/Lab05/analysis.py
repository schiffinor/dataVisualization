...
class Analysis:
    ...
    @staticmethod
    def l1_norm_ndim(a: np.ndarray, b: np.ndarray):
        """
    ...
    @staticmethod
    def l2_norm_ndim(a: np.ndarray, b: np.ndarray):
        """
    ...
    @staticmethod
    def lp_norm(a: np.ndarray, b: np.ndarray, p: int = 2):
        if a.shape != b.shape and a.shape != b.T.shape:
            raise ValueError("Arrays must have the same shape")
        if a.ndim != 1:
            raise ValueError("Arrays must be 1D")
        ...
        ein_string = "i,"+",".join(["i" for j in range(p-1)])+"->"
        diff_copies = [np.abs(b - a) for i in range(p)]
        return np.float_power(np.einsum(ein_string, *diff_copies), inv_p)


    @staticmethod
    def lp_norm_ndim(a: np.ndarray, b: np.ndarray, p: int = 2):
        """
    ...
    @staticmethod
    def l_inf_norm_ndim(a: np.ndarray, b: np.ndarray):
        """
